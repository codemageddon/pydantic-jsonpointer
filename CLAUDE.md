...
- Run tests: `uv run pytest` (tests live under `tests/`: `test_types.py`, `test_errors.py`, `test_adapters.py`, `test_pydantic_adapter.py`, `test_traversal.py`)
...
- Built-in `DictAdapter` and `ListAdapter` self-register at module load. `ListAdapter.resolve_token` enforces RFC 6901 array index shape: ASCII decimal digits only (rejects non-ASCII digits like Arabic-Indic via `_is_ascii_digits`), no leading zero except single `"0"`, plus the `"-"` tail marker. Index helpers reject `bool` keys via `_is_int_index` (since `isinstance(True, int)` would otherwise silently accept `True`/`False` as `1`/`0`).
